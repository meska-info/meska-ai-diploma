...
do $$ begin
  alter table public.leads add constraint leads_years_experience_check
    check (years_experience is null or years_experience in ('0–2 years', '3–6 years', '7–10 years', '10+ years')) not valid;
exception when duplicate_object then null; end $$;

do $$ begin
  alter table public.leads add constraint leads_payment_preference_check
    check (payment_preference is null or payment_preference in ('full', 'installments')) not valid;
exception when duplicate_object then null; end $$;

do $$ begin
  alter table public.leads add constraint leads_start_timing_check
